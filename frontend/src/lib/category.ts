...
import { brandsConfig } from "./getproducts";
...
export async function getProductsByCategory(
  ...
  // Generate products based on brand configurations for this category
  ...
  brandsConfig.forEach(brandConfig => {
    ...
    // Get the category config if it exists for this brand
    const categoryConfig = brandConfig.categories[category];
    ...
    categoryConfig.models.forEach(model => {
      ...
      for (let i = 0; i < variants; i++) {
        ...
        Object.entries(filters).forEach(([key, values]) => {
          if (key !== 'priceRange' && key !== 'brands' && Array.isArray(values) && values.length > 0) {
