...
export default function MarketingTagsPage() {
  const [tags, setTags] = useState<MarketingTag[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");
  const [showPopup, setShowPopup] = useState(false);
  const [editingTag, setEditingTag] = useState<MarketingTag | null>(null);
  const [form, setForm] = useState<TagForm>({ name: "" });
  const [formErrors, setFormErrors] = useState<{ name?: string }>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Fetch all marketing tags
  const fetchTags = async () => {
    try {
      ...
      toast.error("Failed to load marketing tags");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingTag) {
        await updateMarketingTag(editingTag.id, form);
      ...
      toast.error(error.response?.data?.error || "Failed to save marketing tag");
    } finally {
  ...
  // Handle edit
  const handleEdit = (tag: MarketingTag) => {
    setEditingTag(tag);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this marketing tag?")) return;
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Marketing Tags</h1>
        <DefaultButton
      ...
      <Popup
        ...
        title={editingTag ? "Edit Marketing Tag" : "Add Marketing Tag"}
      >
