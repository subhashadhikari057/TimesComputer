...
export default function CategoryPage({ params }: { params: Promise<CategoryParams> }) {
  ...
  // Get sort and page from URL
  const sort = urlSearchParams.get('sort') || undefined;
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const fetchedProducts = await getAllProducts(sort || 'featured', page, 15, {
          ...appliedFilters,
    ...
    fetchProducts();
  }, [sort, page, appliedFilters, resolvedParams.categorySlug]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="lg:hidden flex items-center justify-between mb-4">
          <h1 className="text-2xl font-bold">
            {products.length > 0 
              ? `${resolvedParams.categorySlug.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} Products` 
              : 'No Products Found'
            }
          </h1>
          <button
